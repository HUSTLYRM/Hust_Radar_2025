...
class Topology_lidar:
    ...
    def define_areas(self):
        ...
        for node in map_cfg['nodes']:
            ...
            if node['flag'] == 'red':
             ...
             # 暂时未定义区域划分的具体细节
            # print(self.areas1['6'].vertices)
            print("init area done")
    ...
    # 为车辆分配区域
    def assign_vehicle_to_area(self, id, car_position):
        # 判断并分配车辆到区域，分配的是车辆的id
        x, y, z = car_position
        for area_id, area in self.areas.items():
            # 如果在高地，先加进去
            if area.is_highland and area.contains_point((x, y)):
                self.add_vehicle_to_area(1, area_id)
...
if __name__ == '__main__':
    # 实例化拓扑图
    img = cv2.imread('./RMUC25_map.jpg')
    img = cv2.resize(img,(1400,750))
    cv2.imwrite('C:\Files\E\Radar\Hust-Radar-2024-main\map.jpg',img)
    window = pygame.display.set_mode([1400,1500 // 2])
    ...
    while True:
        ...
        window.blit(bg, (0, 0))
        map_ = Topology_lidar(show_window=window)
        map_.pre_play_with_car(position=[1100,250],id=101)
