...
map_cfg_path = "C:\Files\E\Radar\Hust-Radar-2024-main\points_data.yaml"
...
class MathAnalays:
    ...
    def contain(self,position, polygon_vertices):
        ...
        for i in range(n):
            ...
            if self.on_segment(p1, p2, point):
...
class Area:
    def __init__(self, id, vertices, height=0, is_hightland=False, extended_vertices=None, neighbors=None,
        ...
        self.math_analysis = MathAnalays()
    ...
    def contains_point(self, point):
        ...
        return self.math_analysis.contain((x, y), self.vertices) or self.math_analysis.contain((x, y), self.extended_vertices)
...
class Topology:
    def __init__(self,color):
        ...
        self.window = self.show_window() # 可视化窗口对象，与watcher中的窗口对象保持一致
    ...
    def show_window(self):
        window = pygame.display.set_mode([1280, 679])
        bg = pygame.image.load('RMUC25_map.jpg').convert()
        window.blit(bg, (0, 0))
        return window
    ...
    def define_areas(self):
        ...
        for node in map_cfg['nodes']:
            ...
            if node['flag'] == 'red':
                # 创建区域实例
                self.areas1[node['id']] = Area(node['id'], vertices=np.array(node['corners']), is_hightland=is_highland, height=height)
            else:
                # pass
                self.areas2[node['id']] = Area(node['id'], vertices=np.array(node['corners']), is_hightland=is_highland, height=height)
             # 暂时未定义区域划分的具体细节
    ...
    def pre_play_with_car(self, results):
        self.define_areas()
        print(results)
        ...
        for result in results:
            ...
            for key in self.areas:
                ...
                # print(self.areas1[key])
                polygon_vertices = self.areas[key].vertices
                self.areas[key].draw(self.window, (34, 155, 246), 3)  # 以橙色色线条显示每个区域
                if self.math_analysis.contain(position=position, polygon_vertices=polygon_vertices):
                    self.areas[key].add_vehicle(id)
                    pygame.draw.polygon(self.window, polygon_color, polygon_vertices)

        # pygame.display.update()
        result = self.find_attackable_regions(results)
        print("result", result)
        for value in result.values():
            for area_id in value:
                if area_id > '10':
                    continue
                polygon_vertices = self.areas[area_id].vertices
                pygame.draw.polygon(self.window, (255, 0, 0), polygon_vertices)
        pygame.display.update()
        # self.play_with_car(result)
        # pygame.display.update()
        # time.sleep(10)
        # pygame.quit()
    ...
    def parse_results(self, results):
        # 解析结果并更新车辆信息
        spec_list = {}
        for car_id, position in results:
            if car_id == 1 or car_id == 101:
                region_id = self.find_region(position)
                spec_list["hero"] = region_id if region_id is not None else 0

            elif car_id == 2 or car_id ==102:
                region_id = self.find_region(position)
                spec_list["engine"] = region_id if region_id is not None else 0

            elif ( car_id > 2 and car_id < 7) or (car_id > 102 and car_id < 107):
                region_id = self.find_region(position)
                spec_list["infantry"] = region_id if region_id is not None else 0

            elif car_id == 7 and self.color == 'Blue':
                region_id = self.find_region(position)
                spec_list["sentry"] = region_id if region_id is not None else 0
            elif car_id == 7 and self.color == 'Red':
                pass
            elif car_id == 107 and self.color == 'Red':
                region_id = self.find_region(position)
                spec_list["sentry"] = region_id if region_id is not None else 0
            elif car_id == 107 and self.color == 'Blue':
                pass
            else:
                pass
        return spec_list
    ...
    def find_attackable_regions(self,results):

        # 构建图的邻接列表
        graph = {}
        for edge in map_cfg['attack_edges']:
            source, target_edge = edge['source'], edge['target']
            if source not in graph:
                graph[source] = []
            if target_edge not in graph:
                graph[target_edge] = []
        ...
        # 步骤1: 找到可以攻击目标区域的区域
        ans_dict = {}
        spec_list = self.parse_results(results)

        for key in spec_list.keys():
            target = spec_list[key]
            attackable_regions = []
            for edge in map_cfg['attack_edges']:
                if edge['target'] == target:
                    attackable_regions.append(edge['source'])
                elif edge['source'] == target:
                    attackable_regions.append(edge['target'])
            # 并且和导航点做匹配
            nav_points = [1, 2, 3, 4, 5]
            # check if attackable_region has these points
            ans_points = []
            for region in attackable_regions:
                if region in nav_points:
                    ans_points.append(region)
            ans_dict[key] = attackable_regions
...
if __name__ == '__main__':
    ...
    while True:
        ...
        map_ = Topology('Red')
        results = []
        xy_list = [[359,144,1],[305,326,7],[612,310,2],[418,632,1]]
        # 随机生成100个红色车辆
        for value in xy_list:
            x, y, id = value
            map_.pre_play_with_car([[id,[x,y]]])
        # pygame.display.update()
        time.sleep(10)
