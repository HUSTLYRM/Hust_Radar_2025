...
class Messager:
    def __init__(self , cfg , draw_queue):
        ...
        self.our_hero_area  = cfg["area"]["our_hero_area"]
            ...
            # 英雄辅助吊射
        self.is_assit_hero = True  # 是否协助英雄
        self.hero_xyz = []
        self.hero_assit = Hero_Assit(cfg)
    ...
    # 判断我方英雄是否在指定区域内
    #TODO:修改area
    def is_our_hero_in_area(self,x,y):
        if x < 0 or y < 0 :
            return False
        point = Point(x, y)

        area = self.our_hero_area
        polygon = Polygon(area)
        if polygon.contains(point):
            return True
        return False

    def send_hero_assit(self , x , y):
        if self.is_assit_hero:
            theta_hero = self.hero_assit.get_theta()
            self.send_hero_assit_info = theta_hero
            self.sender.send_hero_assit(self.send_hero_assit_info)
