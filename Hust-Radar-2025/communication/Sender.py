...
6         x    内容数据段    x 最大为 112
    ...
    # receiver_id = self.my_hero_id
    # data = yaw,pitch
    # (1) 获取theta信息
    def generate_hero_assit(self, theta):
        [yaw, pitch] = zip(theta["yaw"], theta["pitch"])
        cmd_id = struct.pack('H', 0x0301)
        data_cmd_id = struct.pack('H', 0x0205)
        sender_id = struct.pack('H', self.my_id)
        receiver_id = struct.pack('H', self.my_hero_id)
        data = data_cmd_id + sender_id + receiver_id + struct.pack('f', yaw) + struct.pack('f', pitch)
        data_len = len(data)
        frame_head = self.get_frame_header(data_len)
    ...
    # (2)英雄辅助吊射发送
    def send_hero_assit(self, theta):
        tx_buff = self.generate_hero_assit(theta)
        self.send_info(tx_buff)
