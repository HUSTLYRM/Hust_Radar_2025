...
# 定义自己的点云处理类
    ...
    #
class PcdQueue(object):# fake queue
    def __init__(self, max_size = 10,voxel_size = 0.05):
        self.max_size = max_size # 入最大次数
        self.queue = deque(maxlen=max_size) # 存放的是np数组
...
class centroidsQueue(object):
    def __init__(self, max_size = 10,voxel_size = 0.05):
        self.max_size = max_size # 入最大次数
        self.queue = deque(maxlen=max_size) # 存放的是np数组类型
        self.pc_all = [] # 用于存储所有中心点的numpy数组
        # 内部属性
        self.record_times = 0 # 已存points的次数
        self.point_num = 0 # 已存点的数量
