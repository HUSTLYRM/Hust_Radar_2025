...
if __name__ == '__main__':
    ...
    try:
        ...
        while True:
            ...
            if infer_result is not None:
                ...
                if results is not None:
                    ...
                    for result in results:
                        ...
                        # 直接拿到赛场定位
                        filter_data = converter.detection_main(new_xyxy_box)
                        detect_results = [now, filter_data, label]
                        location = lidar.
                        # 获取检测框内numpy格式pc
                        ...
                        # 计算距离
                        distance = converter.get_distance(filter_data)
                        if distance == 0:
                            continue
                        # print("xyz:",center,"distance:",distance)

                        # 将点转到赛场坐标系下 ， 此处未完成，返回的是[]
                        field_xyz = converter.camera_to_field(filter_data)
                        # 计算赛场坐标系下的距离
            ...
            messager.update_our_car_infos(my_car_infos)
